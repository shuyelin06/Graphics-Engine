...
namespace Engine
{
DataFilePath::DataFilePath(const std::string& _path)
    : path(_path)
{
    ...
    static const std::unordered_map<std::string, FileExtension> extension_map{
        {"png", PNG}, {"gltf", GLTF}, {"glb", GLB}};
    extension = UNKNOWN;
    if (extension_map.contains(extension_str))
        extension = extension_map.at(extension_str);
}

FileExtension DataFilePath::getExtension() const
{
    return extension;
}
const std::string& DataFilePath::getPath() const
{
    return path;
}
const std::string DataFilePath::getFullPath() const
{
    return "data/" + path;
}
