...
//    If a block already exists, uses the most recently extracted block as the
//    data. Any extracted data is removed from its source.
...
class TextFileReader {
  private:
    ...
    // Stack of blocks of our currently read data. All blocks reference the same
    // source data, but can reference different substring indices of this data.
  ...
  public:
    ...
    // Extracts a block of data from the most recent block (or if none, the
    // file), terminating at the character specified, or until the end of the
    // existing block. Removes any extracted data from its parent, dropping the
    // terminator. Returns true if the parent block was modified, false
    // otherwise.
    ...
    // Pass in -1 for n if we don't want to set a limit on the number of
    // characters to strip.
