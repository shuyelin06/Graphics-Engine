...
Asset* Terrain::getMesh() { return mesh; }
...
void Terrain::generateMesh() {
    /*
       ...
       MarchingCube(terrainData[i][j][k], terrainData[i + 1][j][k],
       terrainData[i
       + 1][j + 1][k], terrainData[i][j + 1][k], terrainData[i][j][k + 1],
       terrainData[i + 1][j][k + 1], terrainData[i + 1][j + 1][k + 1],
       terrainData[i][j + 1][k + 1]);

                                        // Generate triangulation for this cube
                                        marchingCube.generateSurface();
                                        const std::vector<Triangle>&
       triangulation = marchingCube.getTriangles();

                                        // Scale and transform to the voxel
       location Vector3 offset = Vector3(i, j, k) * CHUNK_VOXEL_SIZE;
       ...
       triangulation)
                                        {
                                                ...
                                                meshVertex.position =
       triangle.vertex(0) * CHUNK_VOXEL_SIZE + offset;
                                                meshData->addVertex(meshVertex);
                                                meshVertex.position =
       triangle.vertex(2) * CHUNK_VOXEL_SIZE + offset;
                                                meshData->addVertex(meshVertex);
                                                meshVertex.position =
       triangle.vertex(1) * CHUNK_VOXEL_SIZE + offset;
                                                meshData->addVertex(meshVertex);

                                                MeshTriangle meshTriangle;
                                                meshTriangle.vertex0 =
       curIndex++; meshTriangle.vertex1 = curIndex++; meshTriangle.vertex2 =
       curIndex++; meshData->addTriangle(meshTriangle);
                                        }
                                }
                        }
                }
