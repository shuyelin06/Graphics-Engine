...
void Mesh::parsePLYFile(std::string ply_file, std::string mesh_name)
{
        ...
        // Read header information for layout vertex / face count
        {
                ...
                if (getline(file_stream, line),
                        regex_search(line, match, std::regex("element vertex
(\\d+)")))
                {
                        ...
                        std::regex re_property = std::regex("property float32
([a-z]+)");
                        ...
                        // Parse the properties string to see the intended
layout. Expected order:
                        ...
                        if (properties.substr(0, 3) == "xyz")
                        {
                                ...
                                properties = properties.substr(3,
std::string::npos);
                        }
...
std::string::npos);
                        ...
                        {
                                layout |= NORMAL;
                                properties = properties.substr(6,
std::string::npos);
                        }
                }
                else
                ...
                if (regex_search(line, match, std::regex("element face
(\\d+)")))
                {
...
std::sregex_iterator(line.begin(), line.end(), re_float);

                        while (iter != std::sregex_iterator() && vertex_i <
size)
                        {
                                std::smatch match = *iter;
                                // Add float to corresponding vertex property
position vertex[vertex_i] = stof(match.str(0)); vertex_i++;
        ...
        // Read faces
        {
                ...
                for (int i = 0; i < num_faces; i++)
                {
                        ...
                        // Move through all float matches
                        std::sregex_iterator iter =
std::sregex_iterator(line.begin(), line.end(), re_index);
