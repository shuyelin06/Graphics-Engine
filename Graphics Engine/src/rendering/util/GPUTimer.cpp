...
IGPUTimer::IGPUTimer(GPUTimerBatch* _batch,
                     uint64_t _frame,
...
void GPUTimer::Initialize(ID3D11Device* _device, ID3D11DeviceContext* _context)
{
    device = _device;
    context = _context;
...
void GPUTimer::DisplayGPUTimes()
{
    ...
    const auto result = context->GetData(disjoint_queries[target_query],
                                         &tsDisjoint, sizeof(tsDisjoint), 0);
    if (!SUCCEEDED(result))
        return;
