...
struct AtlasAllocation {
    ...
    AtlasAllocation(UINT x, UINT y, UINT w, UINT h);
    AtlasAllocation();
...
class TextureAtlas {
  private:
    ...
    std::vector<AtlasAllocation> allocations;

    // Used in the 2D Rectangle Packing (see allocateTexture())
    std::vector<AtlasAllocation> open_regions;
  ...
  public:
    ...
    const AtlasAllocation& getAllocation(UINT index) const;
