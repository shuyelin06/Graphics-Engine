...
void ShaderManager::initialize(ID3D11Device* device) {
    ...
    vertexShaders[VSDebugPoint] = createVertexShader(
        device, XYZ | INSTANCE_ID, shaderFolder + L"DebugPointRenderer.hlsl",
        "vs_main");
    vertexShaders[VSDebugPoint]->enableCB(CB0);
    vertexShaders[VSDebugPoint]->enableCB(CB1);
    pixelShaders[PSDebugPoint] = createPixelShader(
        device, shaderFolder + L"DebugPointRenderer.hlsl", "ps_main");

    vertexShaders[VSDebugLine] = createVertexShader(
        device, XYZ | RGB, shaderFolder + L"DebugLineRenderer.hlsl", "vs_main");
    vertexShaders[VSDebugLine]->enableCB(CB1);
    pixelShaders[PSDebugLine] = createPixelShader(
        device, shaderFolder + L"DebugLineRenderer.hlsl", "ps_main");

    vertexShaders[VSDefault] =
        createVertexShader(device, XYZ | TEX | NORMAL,
                           shaderFolder + L"VertexShader.hlsl", "vs_main");
    vertexShaders[VSDefault]->enableCB(CB1);
    vertexShaders[VSDefault]->enableCB(CB2);
    pixelShaders[PSDefault] = createPixelShader(
        device, shaderFolder + L"PixelShader.hlsl", "ps_main");

    vertexShaders[VSShadow] =
        createVertexShader(device, XYZ | TEX | NORMAL,
                           shaderFolder + L"ShadowShaderV.hlsl", "vs_main");
...
static ID3DBlob* CompileShaderBlob(ShaderType type, const std::wstring file,
                                   const char* entry) {
    ...
    HRESULT result = D3DCompileFromFile(file.c_str(), nullptr, include_settings,
                                        entry, compiler_target, flags, 0,
...
VertexShader* ShaderManager::createVertexShader(ID3D11Device* device,
                                                int layout,
...
PixelShader* ShaderManager::createPixelShader(ID3D11Device* device,
                                              const std::wstring filename,
                                              const char* entrypoint) {
    // Obtain shader blob
