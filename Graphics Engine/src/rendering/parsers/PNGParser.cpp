...
namespace Graphics
{
    ...
    Texture* AssetManager::LoadTextureFromPNG(std::string path, std::string png_file)
    {
        ...
        while (readPNGChunk(file, chunk), memcmp(chunk.chunk_type, "IEND", 4) != 0)
        {
            if (memcmp(chunk.chunk_type, "IDAT", 4) == 0)
            {
                raw_data.insert(raw_data.end(), chunk.chunk_data.begin(), chunk.chunk_data.end());
            }
        ...
        uncompress((Bytef *) &decompressed[0], &decompressed_size, (Bytef *) &raw_data[0], raw_size);
    ...
    // 4) UNUSED: A 4-byte CRC calculated on fields (2) and (3), to check for data corruption.
