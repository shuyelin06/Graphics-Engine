...
// Light Class:
// Represents a directional light. Lights create shadows, and we generate these shadows
// shadow mapping.
// The "direction" of the light's view is given by the direction of its rotated
// +Z axis. Lights have a position and rotation in space that defines what regions
// they light up. 
class ShadowLight {
  private:
    Color color;
  ...
  public:
    ...
    void setOrthogonalFrustum(float size_y, float aspect_ratio, float z_near, float z_far);
    void setPerspectiveFrustum(float fov_y, float aspect_ratio, float z_near, float z_far);
