...
void Technique::uploadVertexCBData(uint8_t slot,
                                   const void* src,
                                   size_t byteSize)
{
    assert(slot <= kVertexConstantBufferMax);
...
void Technique::uploadPixelCBData(uint8_t slot,
                                  const void* src,
                                  size_t byteSize)
{
    assert(slot <= kPixelConstantBufferMax);
...
void Technique::bindVertexResource(uint8_t slot, const ShaderResource& resource)
{
    assert(slot <= kVertexResourceMax);
    vResources[slot] = resource;
}
void Technique::bindPixelResource(uint8_t slot, const ShaderResource& resource)
{
